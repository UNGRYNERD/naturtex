<?php
	...
	/*add_action( 'after_setup_theme', function () {
		...
		add_image_size( 'woo-variation-swatch-tooltip-image', $tooltip_image_width, $tooltip_image_width );
	} );*/
			...
			array(
				...
				'fields' => apply_filters( 'wvs_pro_archive_setting_fields', array(
					...
					array(
						...
						'desc'    => sprintf( __( 'Archive product wrapper selector. Default: %s.', 'woo-variation-swatches-pro' ), '<code>.wvs-pro-product</code>' ),
					...
					array(
						...
						'desc'    => esc_html__( 'Archive add to cart button selector. Leave blank for default.', 'woo-variation-swatches-pro' ),
						'default' => ''
					),
	...
	add_filter( 'wvs_simple_setting_fields', function ( $fields ) {
	...
	add_filter( 'wvs_no_individual_settings', function ( $default, $args, $is_default_to_image, $is_default_to_button ) {
	...
	add_filter( 'wvs_variation_attribute_options_html', function ( $data, $args, $is_default_to_image, $is_default_to_button ) {
	...
	add_filter( 'wvs_is_default_to_image', function ( $current, $args ) {
		$product          = $args[ 'product' ];
		$saved_attributes = wvs_pro_get_product_option( $product->get_id() );
	...
	add_filter( 'wvs_default_image_type_attribute', function ( $max_attribute_key, $args ) {
		$product          = $args[ 'product' ];
		$saved_attributes = wvs_pro_get_product_option( $product->get_id() );
	...
	add_filter( 'wvs_is_default_to_button', function ( $current, $args ) {
		$product          = $args[ 'product' ];
		$saved_attributes = wvs_pro_get_product_option( $product->get_id() );
	...
	add_filter( 'woo_variation_swatches_js_options', function ( $options ) {
		...
		$options[ 'archive_add_to_cart_button_selector' ] = apply_filters( 'woo_variation_swatches_archive_add_to_cart_button_selector', trim( woo_variation_swatches()->get_option( 'archive_add_to_cart_button_selector' ) ) );
	...
	add_action( 'wvs_pro_save_product_attributes', function ( $product_id ) {
	...
	add_action( 'wvs_pro_reset_product_attributes', function ( $product_id ) {
		$product        = wc_get_product( $product_id );
		$attribute_keys = array_keys( $product->get_variation_attributes() );
		foreach ( $attribute_keys as $attribute_id ) {
			$archive_transient_name = 'wvs_attribute_html_archive_' . $product_id . "_" . wc_variation_attribute_name( $attribute_id );
